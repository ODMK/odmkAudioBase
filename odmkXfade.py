...
def tablegen(depth):
    ''' generate a look-up table with depth = "depth"
       creates a look-up table entries for different waveforms '''

    # tl = np.linspace(0, 1/(depth-1), 1)    # ?? check this linspace
    tl = np.linspace(0.0, 1.0, depth)
    # store 1 cycle
    for q in range(len(tl)):
        # 2 multiple necessary because function range changes from [0:2] to [0:1] for normalization
        table1[q] = np.cos(2 * np.pi/4 * tl[q])
...
# Original Scilab code - reference
